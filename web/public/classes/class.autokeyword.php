...
class autokeyword {
	...
	{
		$keywords = $this->parse_2words().$this->parse_words().$this->parse_3words();
	...
	function parse_words()
	{
		...
		foreach( $s as $key=>$val ) {
			...
			if(mb_strlen(trim($val)) >= $this->wordLengthMin  && !in_array(trim($val), $this->common)  && !is_numeric(trim($val))) {
				$k[] = trim($val);
			}
		}
		//count the words
	...
	function parse_2words()
	{
		...
		for ($i=0; $i < $count; $i++) {
			...
			if( (mb_strlen(trim($x[$i])) >= $this->word2WordPhraseLengthMin ) && (mb_strlen(trim($x[$i+1])) >= $this->word2WordPhraseLengthMin) && !in_array(trim($x[$i]), $this->common) && !in_array(trim($x[$i]+1), $this->common))
			{
