...
class phpthumb_functions {
		...
		do {
			...
			if ($fp = @fsockopen($host, 80, $errno, $errstr, $timeout)) {
				...
				while (!feof($fp)) {
					...
					if (preg_match('^/HTTP/[\\.0-9]+\s([0-9]+)\s(.+)$/i', rtrim($line), $matches)) {
						list($dummy, $errno, $errstr) = $matches;
						$errno = intval($errno);
					} elseif (preg_match('/^Location: (.*)$/i', rtrim($line), $matches)) {
					//} elseif (stristr($line,'location:')!='') {
						$header_newlocation = $matches[1];
						//$header_newlocation = trim(preg_replace('/location:/i','',$line));
					}
					if ($isHeader && ($line == "\r\n")) {
						...
						if ($successonly) {
							switch ($errno) {
								...
								default:
									$errstr = $errno.' '.str_replace('Permantenly','Permanently',$errstr).($header_newlocation ? '; Location: '.$header_newlocation : '');
									fclose($fp);
				...
				if ($header_newlocation) {
					$parsed_url = phpthumb_functions::ParseURLbetter($header_newlocation);
					$host = (string)$parsed_url['host'];
					$file = (string)$parsed_url['path'].($parsed_url['query'] ? '?'.$parsed_url['query'] : ''); 
				}
			} else return null;
			$ctr++;
		} while ($header_newlocation && $ctr < 3); //2 redirects are enough.
	...
	function SafeURLread($url, &$error, $timeout=10, $followredirects=true) {
		...
		while (true) {
			$tryagain = false;
			$rawData = phpthumb_functions::URLreadFsock(@$parsed_url['host'], @$parsed_url['path'].'?'.@$parsed_url['query'], $errstr, false, (@$parsed_url['port'] ? @$parsed_url['port'] : 80), $timeout);
