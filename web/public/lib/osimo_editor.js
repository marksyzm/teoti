		...
		$.ajax({
			...
			success:function(resp){
				...
				/*if(window.sessionStorage){
					sessionStorage.osimo_bbeditor_theme = resp;
				}*/
				obj.buildEditor();
			}
		});
...
OsimoEditorControls.prototype.replaceText = function(text,backChar){
	if (backChar == undefined) backChar = false;
	var 
		inputID = '#'+this.input+'_editbox'
		,inEl = $(inputID)
		,replace = inEl.attr('value').substring(0,text.start) + text.replace + text.textarea.value.substring(text.end,text.len);
	console.log(text)
	var crt = text.start + text.replace.length - backChar;//3 = strlen of [/]
	//var crt = inEl.caret()-backChar-3;
	inEl.attr('value',replace);
	var crtPos = crt;
	inEl.focus();
	
	if (backChar != false)
		inEl.textSelection(crtPos-text.sel.length,crtPos);
		//inEl.caret(crtPos); 
}
...
OsimoEditorControls.prototype.code = function(){
	this.simpleReplace('code');	
}

OsimoEditorControls.prototype.media = function(){
	this.simpleReplace('media');	
}
